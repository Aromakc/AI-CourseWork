...
def calculateMisplacedTiles(data):
    global goalStateData
    count = 0
    for i in range(3):
        for j in range(3):
            if data[i][j] != goalStateData[i][j]:
                count = count + 1
    return count
...
def performAction(state, i, j, action):
    ...
    newState["misplacedTiles"] = calculateMisplacedTiles(newData)
    newState["manhattanDistance"] = newState["misplacedTiles"] + newState["level"]
...
def stateSpaceSearch():
    ...
    while not reachedGoal:
         # Extract Manhattan distances from each item
        distances = [item["manhattanDistance"] for item in stateSpaceTree[-1]]
        # Find the item with the smallest distance
        min_distance_index = distances.index(min(distances))
        item = stateSpaceTree[-1][min_distance_index]
    ...
    for state in stateSpaceTree:
        for item in state:
            ...
            tree.node(str(item["id"]), f"{data[0][0]}|{data[0][1]}|{data[0][2]}\n{data[1][0]}|{data[1][1]}|{data[1][2]}\n{data[2][0]}|{data[2][1]}|{data[2][2]}\n \ng→{item['level']} h→{item['misplacedTiles']} f→{item['manhattanDistance']}", color =getColor)
            if item["parentId"] != '':
                tree.edge(str(item["parentId"]), str(item["id"]), label = str(item["action"]))
    tree.render('test-output/8PuzzleHeuristic.gv', view=True)
